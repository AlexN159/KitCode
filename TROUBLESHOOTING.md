...
Normal coach chat adapts to what you ask; there are no separate Hint/Teach/Show modes. **Give me a hint** makes an explicit coach request and displays a short, read-only `# Hint:` beside a relevant editor line. It does not modify the script. The hint remains visible through typing, reset, run, trace, and submit until you select **Dismiss**. Switching drills or AI providers clears it because the advice belongs to the previous context.
